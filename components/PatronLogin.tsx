...
const PatronLogin: React.FC<PatronLoginProps> = ({ onLogin, onNavigateToLogin, onNavigateToSignUp }) => {
  ...
  const handleQuickLogin = async () => {
    ...
    try {
      ...
      await onLogin('patron@clubhub.local', 'password');
    } catch (err: any) {
      setError(err.message || 'Quick login failed. There might be an issue with the backend service.');
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <div className="flex items-center justify-center min-h-screen bg-gray-100 dark:bg-gray-900">
      <div className="w-full max-w-sm p-8 space-y-6 bg-white dark:bg-gray-800 rounded-xl shadow-lg">
        ...
        <div className="space-y-4">
            ...
            >
                {isLoading ? 'Please wait...' : 'Quick Login with Demo Patron'}
            </button>
            
            <details className="text-sm text-gray-600 dark:text-gray-400">
                <summary className="cursor-pointer hover:text-purple-500 text-center">Or log in manually</summary>
                <form onSubmit={handleSubmit} className="space-y-6 mt-4">
                  ...
                  <div>
                    ...
                    >
                      {isLoading ? 'Logging in...' : 'Log In Manually'}
                    </button>
                  </div>
                </form>
            </details>
